...
class TealAnimatedButton : androidx.appcompat.widget.AppCompatButton {
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        if (event != null) {
            ...
            if (event.action == MotionEvent.ACTION_DOWN) {
                ...
                setBackgroundResource(R.drawable.teal_scanner_button_pressed)
            } else if (event.action == MotionEvent.ACTION_UP || event.action == MotionEvent.ACTION_CANCEL) {
                setPadding(0, dpAsPixels20, 0, dpAsPixels30)
                setBackgroundResource(R.drawable.teal_scanner_button)
            }
        }
